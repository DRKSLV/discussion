    ...
    value: function checkActive() {
      ...
      if (this.state.id.active !== null) {
        this.state.axios.get("/protected").then(function (val) {
          ...
          if (reason.response) {
            var tokens = reason.response.headers["www-authenticate"].split(",").map(function (x) {
              return x.trim();
            }); //decide what to do

            if (tokens[1] && tokens[1].startsWith('error="')) {
              var error = tokens[1].substring(7, tokens[0].length - 2);
...
//# sourceMappingURL=index.js.ca5ab84f3b6aa3ceb630.hot-update.js.map